# kubeSol/core/context/kubesol_context.py
"""
Manages the current operational context (project, environment, namespace) for KubeSol.
An instance of KubeSolContext will be managed by the main shell and provided to
all command handlers and plugins.
"""
...
class KubeSolContext:
    ...
    def _update_prompt_prefix(self):
        ...
        elif self.current_namespace and self.current_namespace != self.default_namespace:
            self.prompt_prefix = f"({self.current_namespace})"
        else:
            self.prompt_prefix = f"({self.default_namespace})"
    ...
    def clear_project_context(self):
        ...
        self.project_id = None
        self.environment_name = None
        self.current_namespace = self.default_namespace # Revert to default namespace
        self._update_prompt_prefix()
